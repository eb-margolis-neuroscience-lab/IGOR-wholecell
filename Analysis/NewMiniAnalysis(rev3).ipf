...
// Mini detection settings		
	...
	Variable /G min_amplitude = 4			// detection threshold, pA  (positive number)
...
Window Mini_analysis_window() : Graph
	...
	Cursor A current1wave 0.001;Cursor B current1wave 0.002
...
//puts cursors on the mini and moves the graph to center it
	...
	if (mini_start_time > (rightlimit-0.05))   //moves window over to keep up with the current trace
		leftlimit = mini_start_time -0.05
		rightlimit = mini_start_time +0.25
		SetAxis bottom leftlimit, rightlimit
	endif
